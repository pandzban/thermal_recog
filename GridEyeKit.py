...
class GridEYEKit():
    def __init__(self):
        ...
        #    print( "please connect Eval Kit")#     print( "please connect Eval Kit")
    ...
    def connect(self):
        ...
        if self.ser.isOpen():
            ...
            for port in ports_available:
                self.ser = serial.Serial(port=port, baudrate=57600, # 57600
                                         timeout=0.5)  # COM Port error is handled in list serial ports
                for i in range(5): # 5
                    if self.serial_readline(bytes_timeout=300):  # if 3 bytes identifyer found 300
                        self._connected = True
                        return True  # GridEye fo
# und
                self.ser.close()
            self._connected = False
            return False
        ...
        if len(data) >= 135:
            ...
            for i in range(2, 130, 2):  # 2,130
                ...
                if not data[i + 1] & 0b00001000 == 0:  # Grid-Eye uses 12 bit two's complement for calculating data
                    data[
                        i + 1] |= 0b11111000  # if 12 bit complement, set bits 12 to 16 to convert to 16 bit two's complement
                tarr[r][c] = struct.unpack('<h', data[i:i + 2])[
    ...
    def serial_readline(self, eol='***', bytes_timeout=300):
        ...
        while True:
            ...
            if c:
                ...
                if line2[-length:] == eol:
                    break
                if len(line2) > bytes_timeout:  # timeout
                    return []
            else:
                break
        #print("line", line)
